...
def parse_lotl(lotl_content: bytes) -> list[tuple[str, str]]:
    ...
    for tsp in root.findall(".//tsl:OtherTSLPointer", NAMESPACE):
        ...
        if url_elem is None or country_code_elem is None:
            continue

        url_text = url_elem.text
        code_text = country_code_elem.text

        if url_text is None or code_text is None:
            continue
        if code_text == "EU":
            continue

        result.append((code_text, url_text))
...
def update_all_tsl_entries() -> list[dict[str, str]]:
    ...
    for country_code, url in entries:
        ...
        log_rows.append({"Country": country_code, "URL": url, "Status": status, "FileSaved": "Yes" if saved else "No"})
